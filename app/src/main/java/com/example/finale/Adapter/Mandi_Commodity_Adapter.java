...
import com.example.finale.Mandi_Details_Activity;
import com.example.finale.Mandi_Records;
import com.example.finale.R;

import java.util.List;

public class Mandi_Commodity_Adapter extends ArrayAdapter<Mandi_Records> {
    ...
    public Mandi_Commodity_Adapter(List<Mandi_Records> heroList, Context mCtx) {
...
//                if (!preferences.getString("DistrictName", "").equals(heroList.get(j).getDistrict())) {
//                    heroList.remove(j);
//                    j--;
//                }
//            }
//        }
        for (int i = 0; i < heroList.size(); i++) {
            ...
            //if (!preferences.getString("StateName", "").equals(heroList.get(j).getState())) {
            if (!preferences.getString("DistrictName", "").equals(heroList.get(i).getDistrict())) {
                heroList.remove(i);
                i--;

            }else {
                for (int j = i +1; j < heroList.size(); j++) {
                    if (heroList.get(i).getCommodity().contains(heroList.get(j).getCommodity())) {
                        heroList.remove(j);
                        j--;
                    }
                }
        ...
        textViewName.setText(heroList.get(position).getCommodity());
        recordsLists.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                SharedPreferences preferences = PreferenceManager.getDefaultSharedPreferences(getContext());
                SharedPreferences.Editor editor = preferences.edit();
                editor.putString("CommodityName",heroList.get(position).getCommodity());
                System.out.println(preferences.getString("CommodityName", ""));
                editor.apply();
                Intent intent = new Intent(getContext().getApplicationContext(), Mandi_Details_Activity.class);
